...
bool VmHandleInvokeSuper(VmContext &ctx, const VmpInstruction &insn) {
    ...
    int argCount = static_cast<int>(insn.registers.size()) - 1;

    ctx.lastResultObject = nullptr;
    ctx.lastResultInt = 0;
    ctx.lastResultLong = 0;

    if (isVoidReturn(signature)) {
        if (argCount == 0) {
            ctx.env->CallNonvirtualVoidMethod(obj, superCls, mid);
        } else if (argCount == 1) {
            jobject a0 = ctx.regs[insn.registers[1]].objectValue;
            ctx.env->CallNonvirtualVoidMethod(obj, superCls, mid, a0);
        } else if (argCount == 2) {
            jobject a0 = ctx.regs[insn.registers[1]].objectValue;
        ...
        if (argCount == 0) {
            ctx.lastResultObject = ctx.env->CallNonvirtualObjectMethod(obj, superCls, mid);
        } else if (argCount == 1) {
            jobject a0 = ctx.regs[insn.registers[1]].objectValue;
            ctx.lastResultObject = ctx.env->CallNonvirtualObjectMethod(obj, superCls, mid, a0);
        } else if (argCount == 2) {
            jobject a0 = ctx.regs[insn.registers[1]].objectValue;
            jobject a1 = ctx.regs[insn.registers[2]].objectValue;
